var updater = {
   ...
   monitor: function () {
      setInterval(function() {
         ...
         $('.progress').stop().animate({width: per + "%"}, 700);
   ...
   retrieve: function () {
      ...
      $.getJSON('http://api.openweathermap.org/data/2.5/weather', weather, function (json, textStatus) {
         ...
         if (json.sys == null) {
            json = {name: "City", sys: {sunrise: 0, sunset: 0}, weather:{icon:"wi-cloudy", description: "orrrr"}, main:{temp_min: 10, temp_max: 20, temp: 18}};
         } else {
